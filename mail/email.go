...
func NewEmailFromReader(r io.Reader) (*Email, error) {
	...
	for _, p := range ps {
		headerContentType := p.header.Get(CONTENT_TYPE)
...
func (e *Email) Bytes() ([]byte, error) {
	...
	switch {
	case isMixed:
		headers.Set(CONTENT_TYPE, "multipart/mixed;\r\n boundary="+w.Boundary())
	case isAlternative:
		headers.Set(CONTENT_TYPE, "multipart/alternative;\r\n boundary="+w.Boundary())
	case isRelated:
		headers.Set(CONTENT_TYPE, "multipart/related;\r\n boundary="+w.Boundary())
	case len(e.Html) > 0:
		headers.Set(CONTENT_TYPE, "text/html; charset=UTF-8")
		headers.Set(CONTENT_TRANSFER_ENCODING, QUOTED_PRINTABLE)
	default:
